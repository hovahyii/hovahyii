...
import { ArrowLeft, ArrowRight, Bot, CalendarClock, Check, ChevronRight, Cloud, Code2, Database, Download, ExternalLink, FileKey, Globe2, KeyRound, Layers3, LockKeyhole, MessageCircle, MousePointer2, Play, Radio, RefreshCw, Rocket, Server, ShieldCheck, Sparkles, Trash2, Upload, Users, Wifi, Zap } from "lucide-react";
...
export default function Home() {
  ...
  return <main className="deck" onTouchStart={(e) => { touchStart.current = e.touches[0]?.clientX ?? null; }} onTouchEnd={(e) => { if (touchStart.current === null) return; const distance = (e.changedTouches[0]?.clientX ?? touchStart.current) - touchStart.current; if (Math.abs(distance) > 55) go(slide + (distance < 0 ? 1 : -1)); touchStart.current = null; }}>
    ...
    <section ref={stageRef} className="stage" aria-live="polite">
      ...
      {slide === 8 && <article className="slide"><Heading step="08 · STORAGE CDN" title="Store once. Deliver from the edge." text="Every Storage request reaches the CDN first. Public files get better cache reuse; private files trade some cache efficiency for per-user authorization."/><div className="two-col"><div className="cdn-map"><div className="map-label"><Globe2/>CDN EDGE NETWORK</div><div className="origin"><Cloud/><b>Supabase Storage</b><small>origin · Singapore</small></div><div className="edge e1"><span>MY</span><small>edge</small></div><div className="edge e2"><span>US</span><small>edge</small></div><div className="edge e3"><span>EU</span><small>edge</small></div><div className={"packet request-" + Math.min(requests,2)}/><p>{requests === 0 ? "Choose a bucket and request an asset." : requests === 1 ? "First request: CDN cache MISS → fetch origin." : "Next request: CDN cache HIT → serve from edge."}</p></div><div className="bucket-panel"><div className="bucket-tabs"><button className={bucketMode === "public" ? "active" : ""} onClick={() => {setBucketMode("public");setRequests(0)}}>Public bucket</button><button className={bucketMode === "private" ? "active" : ""} onClick={() => {setBucketMode("private");setRequests(0)}}>Private bucket</button></div><div className="bucket-icon">{bucketMode === "public" ? <Globe2/> : <FileKey/>}</div><h3>{bucketMode === "public" ? "Product images & public assets" : "User documents & private media"}</h3><p>{bucketMode === "public" ? "Anyone with the URL can download. Best cache hit rate and simplest delivery." : "Access is checked per user. Serve with authenticated downloads or signed URLs."}</p><ul><li><Check/>{bucketMode === "public" ? "getPublicUrl()" : "RLS or signed URL"}</li><li><Check/>{bucketMode === "public" ? "High CDN cache reuse" : "Per-user authorization"}</li></ul><button onClick={() => setRequests(Math.min(requests + 1,2))}>Request asset <ArrowRight/></button></div></div></article>}

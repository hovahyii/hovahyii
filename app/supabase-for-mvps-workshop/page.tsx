...
const PROJECT_URL = "https://xkimiccvqhhjzagojvvv.supabase.co";
const PUBLISHABLE_KEY = "sb_publishable_DLGIy9GhuPpK5UzhH8YYSw_Cz2j-jt8";
...
const chapters = [
  ...
  ["Build", "Your practical MVP stack"],
];
...
const queryCode = ['const { data, error } = await supabase', '  .from("mvp_ideas")', '  .select("name, pitch, votes")', '  .order("votes", { ascending: false })', '', 'if (error) throw error'].join("\n");
...
export default function Home() {
  ...
  useEffect(() => {
    const saved = window.localStorage.getItem("mranti-workshop-theme");
  ...
  const toggleTheme = () => setTheme((current) => {
    ...
    window.localStorage.setItem("mranti-workshop-theme", next);
    return next;
  });
  ...
  const testConnection = async () => {
    ...
    if (error) { setConnection("error"); setDataStage(0); return; }
    setIdeas((data as Idea[]) ?? fallbackIdeas);
    setRowCount(count ?? data?.length ?? 0);
    setConnection("live");
    setDataStage(3);
  };
  ...
  return <main className={"deck theme-" + theme} onTouchStart={(e) => { touchStart.current = e.touches[0]?.clientX ?? null; }} onTouchEnd={(e) => { if (touchStart.current === null) return; const distance = (e.changedTouches[0]?.clientX ?? touchStart.current) - touchStart.current; if (Math.abs(distance) > 55) go(slide + (distance < 0 ? 1 : -1)); touchStart.current = null; }}>
    <header className="deck-header">
      <button className="brand brand-lockup" onClick={() => go(0)} aria-label="MRANTI and Supabase Kuala Lumpur — go to first slide"><img className="mranti-logo" src="/supabase-for-mvps-workshop-assets/mranti-30-logo.png" alt="MRANTI 30 Years"/><i>×</i><span className="supabase-kl-logo"><img src="/supabase-for-mvps-workshop-assets/supabase-kl-logo.png" alt="Supabase Kuala Lumpur"/></span></button>
      ...
      <button className="theme-toggle" onClick={toggleTheme} aria-label={"Switch to " + (theme === "dark" ? "light" : "dark") + " mode"}>{theme === "dark" ? <Sun/> : <Moon/>}<span>{theme === "dark" ? "Light" : "Dark"}</span></button>
    ...
    <section ref={stageRef} className="stage" aria-live="polite">
      {slide === 0 && <article className="slide hero-slide">
        <div className="slide-copy"><span className="eyebrow"><Sparkles size={14}/> 01 · WHY SUPABASE · INTERACTIVE DEMO</span><h1>One database.<br/><em>Less backend glue.</em></h1><p className="lead">Supabase is managed Postgres with the backend services an MVP usually needs—available through one dashboard and one SDK.</p><div className="hero-points"><span><Database/>Real Postgres</span><span><Code2/>Generated API</span><span><ShieldCheck/>Policies near the data</span></div><button className="primary-action" onClick={() => go(1)}>Show me how <ArrowRight size={18}/></button></div>
